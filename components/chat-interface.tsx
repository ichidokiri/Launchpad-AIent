...
export default function ChatInterface() {
    ...
    const showMoreOptions = () => {
        toast.success("More options will be available soon")
    }
                    ...
                    {messages.map((message) => (
                        <div key={message.id} className="py-6">
                            <div className={`flex gap-4 ${message.isUser ? "flex-row-reverse" : ""}`}>
                                {!message.isUser && (
                                    <div className="relative flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-foreground">
                                        <svg
                                            width="24"
                                            height="24"
                                            viewBox="0 0 41 41"
                                ...
                                <div className={`flex-1 space-y-2 ${message.isUser ? "flex justify-end" : ""}`}>
                                    <div
                                        className={
                                            message.isUser
                                                ? "bg-primary text-primary-foreground rounded-lg px-4 py-2 inline-block"
                                                : "bg-card rounded-lg px-4 py-2 inline-block"
                                        }
                                    >
                        ...
                        {line}
                                    ...
                                    {!message.isUser && (
                                        <div className="flex items-center gap-2 text-muted-foreground">
                                            ...
                                            <button
                                                className={`p-1 hover:text-foreground transition-colors ${message.feedback === "liked" ? "text-green-500" : ""}`}
                                            ...
                                            <button
                                                className={`p-1 hover:text-foreground transition-colors ${message.feedback === "disliked" ? "text-red-500" : ""}`}
                                            ...
                                            <button
                                                className={`p-1 hover:text-foreground transition-colors ${isRegenerating ? "animate-spin text-blue-500" : ""}`}
