...
// Update the prisma instance export to handle the model name mismatch
// Add this function to get the correct model name

/**
 * Gets the correct model name for Prisma
 * This helps handle the case sensitivity issues between model definitions and generated client
 * @param modelName The model name as defined in the schema
 * @returns The correct model name to use with Prisma client
 */
export function getModelName(modelName: string): string {
  ...
  // Try camelCase version (AIAgent -> aIAgent)
  ...
  const allModelNames = Object.keys(prisma).filter(
      (key) =>
  ...
  const match = allModelNames.find((name) => name.toLowerCase() === modelName.toLowerCase())

  if (match) {
    return match
  }

  // If no match found, return the original (will likely cause an error, but at least we tried)
  console.warn(`Model name "${modelName}" not found in Prisma client. Available models: ${allModelNames.join(", ")}`)
  return modelName
}
